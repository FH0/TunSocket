...
iptables -t mangle -D OUTPUT -p tcp -d 192.168.121.113 -m owner --uid-owner fh0 -j CONNMARK --set-xmark 0x3333
ip rule del fwmark 0x3333 lookup 3333
ip route flush table 3333
...
if [ "$1" = "start" ]; then
    ...
    iptables -t mangle -I OUTPUT -p tcp -d 192.168.121.113 -m owner --uid-owner fh0 -j CONNMARK --set-xmark 0x3333
