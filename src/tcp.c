    ...
    for (tmp = tcpHead->tcp.next; tmp; tmp = tmp->tcp.next) {
        ...
        if (payloadLen) {
            ...
            if (!(tmp->tcp.status & 0xf0)) {
                ...
                memcpy(&tmp->tcp.rBuf[tmp->tcp.rBufLen],
            ...
            if ((tcpET == 1) && (!(tmp->tcp.status & 0xf0))) {
                ts_cb(tmp);
            }
        }
        /* ack */
...
void tcp_write(ts_data_t *data) {
    ...
    memcpy(&bufTmp[iphdrLen + tcphdrLen], data->tcp.wBuf,
           bufTmpLen - iphdrLen - tcphdrLen);
    if (data->tcp.window <
        data->tcp.wBufLen) /* no buffer left, so no need to memmove */
    {
        memmove(data->tcp.wBuf,
                &data->tcp.wBuf[bufTmpLen - iphdrLen - tcphdrLen],
    ...
    if (flag == 0x00) {
        if ((tunMtu - iphdrLen - tcphdrLen) > 65535) {
            *(int *)&bufTmp[iphdrLen + tcphdrLen - 4] =
                htonl(0x02040000 + 65535);
        } else {
            *(int *)&bufTmp[iphdrLen + tcphdrLen - 4] =
                htonl(0x02040000 + tunMtu - iphdrLen - tcphdrLen);
        }
    }
