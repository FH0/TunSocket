...
void ts_udp_write(char flag, void *sip, uint16_t sport, void *dip, uint16_t dport, void *buf, int bufLen)
{
    ...
    if (flag == TS_UDP)
    {
        ...
        *(uint16_t *)&bufChecksum[10] = htons(bufTmpLen - iphdrLen);
        ...
        udp->check = calculate_checksum((uint16_t *)bufChecksum, 12 + bufTmpLen - iphdrLen);
    }
    else
    {
        ...
        *(uint16_t *)&bufChecksum[34] = htons(bufTmpLen - iphdrLen);
        ...
        udp->check = calculate_checksum((uint16_t *)bufChecksum, 36 + bufTmpLen - iphdrLen);
    }
